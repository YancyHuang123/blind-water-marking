...
class MainModel(L.LightningModule):  # type: ignore
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log_dict({"loss_D_val": loss_D, 'loss_H_val': loss_H, 'loss_mse_val': loss_mse, 'loss_ssim_val': loss_ssim,
                       'loss_adv_val': loss_adv, 'loss_DNN_val': loss_DNN}, prog_bar=True, on_step=False, on_epoch=True,sync_dist=True)
        self.log_dict({'host_net_no_trigger_acc': self.host_net_no_trigger_acc.compute(), 'host_net_trigged_rate':
                      self.host_net_trigged_rate.compute(), 'host_net_error_trigged_rate': self.host_net_error_trigged_rate.compute(), 'discriminator_acc': self.discriminator_acc.compute()}, prog_bar=True, on_step=False, on_epoch=True,sync_dist=True)
